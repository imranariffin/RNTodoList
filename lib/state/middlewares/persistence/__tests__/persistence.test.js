...
describe('persistence middleware', () => {
  describe('items updated from server', () => {
    const succesAsyncActionsOnItems = [
      ...
      it('should update items in the storage', async () => {
        ...
        expect(storageClient.loadItems).toHaveBeenCalledTimes(0)
  ...
  describe('failure getting items from api client', () => {
    const action = {
    ...
    it('should load items data from local storage', async () => {
      const itemsFromReduxStore = {}
      const itemsFromLocalStorage = {}
      const storageClient = {
        saveItems: jest.fn(),
        loadItems: jest.fn().mockReturnValue(itemsFromLocalStorage)
      }
      const store = {
        getState: jest.fn().mockReturnValue(itemsFromReduxStore),
        dispatch: jest.fn()
      }
      const next = jest.fn(action => action)
      ...
      expect(storageClient.loadItems).toHaveBeenCalledTimes(1)
      ...
      expect(store.dispatch.mock.calls[1][0]).toEqual({
        ...
        data: { items: itemsFromLocalStorage }
      })
    })
  })
